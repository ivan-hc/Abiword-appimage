...
VERSION=$(echo $(wget -q https://packages.debian.org/stable/amd64/abiword/download -O - | grep "abiword_" | tail -1 | grep -o -P '(?<=abiword_).*(?=_amd64.deb)'))

cd ..;
mv ./tmp/*.AppImage ./Abiword-$VERSION-x86_64.AppImage

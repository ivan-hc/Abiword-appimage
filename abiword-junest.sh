...
mkdir -p $HOME/.cache
$HERE/.local/share/junest/bin/junest proot -n -b "--bind=/home --bind=/home/$(echo $USER) --bind=/media --bind=/opt --bind=/usr/share/fonts --bind=/usr/share/themes --bind=/usr/lib/locale --bind=/etc" 2> /dev/null -- $APP "$@" 
EOF
